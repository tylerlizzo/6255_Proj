...
addpath tools
chime_path = '../CHiME3/';
addpath([chime_path '/tools/utils/']);
upath=[chime_path '/data/audio/16kHz/isolated/']; % path to segmented utterances
upath_fn=[chime_path '/data/audio/16kHz/isolated_fn/']; %path to filtered speech and noise
cpath=[chime_path '/data/audio/16kHz/embedded/']; % path to continuous recordings
bpath=[chime_path '/data/audio/16kHz/backgrounds/']; % path to noise backgrounds
apath=[chime_path '/data/annotations/']; % path to JSON annotations
nchan=4;
...
equal_filter = [chime_path '/tools/simulation/equal_filter.mat'];
if exist(equal_filter,'file'),
    load(equal_filter);
else
...
% Create new (non-official) annotations
    ...
    for utt_ind=1:length(mat),
        ...
        while nfail,
            ...
            for c=1:nchan,
                n(:,c)=audioread([bpath nname '.CH' int2str(c+1) '.wav'],[nbeg nend]);
            end
            npow=sum(n.^2,1);
        ...
        while xfail,
            ...
            for c=1:nchan,
                x(:,c)=audioread([cpath iname '.CH' int2str(c+1) '.wav'],[ibeg iend]);
            end
            xpow=sum(x.^2,1);
...
for utt_ind=1:length(mat),
    if official,
        ...
        udir_fn=[upath_fn 'tr05_' lower(mat{utt_ind}.environment) '_simu/'];
    else
    ...
    % Load WAV files
    o=audioread([upath 'tr05_org/' oname '.wav']);%clean speech
    [r,fs]=audioread([cpath iname '.CH0.wav'],[ibeg iend]);%real case CH0
    x=zeros(iend-ibeg+1,nchan);
    n=zeros(nend-nbeg+1,nchan);
    for c=1:nchan,
        x(:,c)=audioread([cpath iname '.CH' int2str(c+1) '.wav'],[ibeg iend]);
    ...
    disp([udir_fn uname]);
    for c=1:nchan,
        audiowrite([udir uname '.CH' int2str(c+1) '.wav'],xsimu(:,c),fs);        
        audiowrite([udir_fn uname '.CH' int2str(c+1) '.fclean.wav'],ysimu(:, c), fs);
        audiowrite([udir_fn uname '.CH' int2str(c+1) '.noise.wav'],n(:, c),fs);
    end
end
...
for set_ind=1:length(sets),
    ...
    for utt_ind=1:length(mat),
        if official,
            ...
            udir_fn=[upath_fn set '_' lower(mat{utt_ind}.environment) '_simu/'];
        else
        ...
        % Load WAV files
        o=audioread([upath set '_bth/' oname '.CH0.wav']);%clean speech
        [r,fs]=audioread([cpath nname '.CH0.wav'],[tbeg tend]);
        nsampl=length(r);
        x=zeros(nsampl,nchan);
        for c=1:nchan,
            x(:,c)=audioread([cpath nname '.CH' int2str(c+1) '.wav'],[tbeg tend]);
        end
